...
\hypersetup{pdfsubject={TT2 Problemstellung 1 : Zusammenfassung}, pdftitle={Reinforcement Learning}, pdfkeywords={Framework, Monte Carlo Methode, Dynamic Programming, Temporal Difference Learning, Reinforcement Learning}}
...
\end{quote}
	...
	Der Gedanke bei Temporal Difference Learning ist, alle Erwartungswerte für rewards beliebig zu füllen, um dann beim 	Durchlaufen eine Pfades, die auf diesem liegenden Werte anhand ihrer Abweichung $real - erwartet$ anzugleichen.
...
Abbildung \ref{fig:Cliff_Result} ist zu entnehmen wie sich mit $\epsilon = 0.1$ der durchschnittliche Reward für Sarsa bzw. Q Learning entwickelt. Es ist festzustellen das bei konstantem $\epsilon$ die von Q Learning ausgeführte exploriation dazu führt, dass der durchschnittliche Reward schlechter als bei Sarsa ist. Das kommt daher, dass zwar der potentiell optimalere Weg direkt am Kliff entlang gelernt wird, aber in der exploriation dann regelmäßig über das cliff hinausgegangen wird. Würde man $\epsilon$ kontinuierlich verringern, würden sich beide Lösungen sich asympthotisch der optimalen anpassen \cite[Vgl.][S. 186,187]{sutton:2005}. \todo{check: wieso passen sie sich an ? OS}
...
Das Vorgehen ist dabei wie folgt: Für jeden besuchten Zustand wird im ET der Wert für den Zustand um 1 erhöht (Zeile 8 im Sarsa($\lambda$) Pseudo-Code, Abb. \ref{fig:SarsaLambda}). Nach jedem Schritt wird der Fehler $\delta$ auf den Zustand aufaddiert (Zeile 10) und der komplette ET wird um den Trace Decay Faktor sowie den Diskountfaktor reduziert (Zeile 11).
...
Monte Carlo Methoden betrachten jeweils nur eine Sequenz von Zustandsübergängen (eine \emph{Experience}) und benötigt daher kein Modell. \todo{Check: Kein Modell oder ein kleineres ? Wir erstellen doch beim TD eigentlich auch ein Modell, wie ist hier die Abgrenzung ? Wir bauen ja mit unserer Erfahrung eine Map was wir in einem Zustand erwarten. Das klingt nach Modell. Siehe Sutton Page 20 (PDF Page), letzter Absatz OS}\\
...
\end{description}
	...
	\bibliographystyle{dinat} % Anpassung an deutsche Zitierweise
  	...
  	\bibliography{Bericht}	% Literaturverzeichnis
  	\input{literatur}
